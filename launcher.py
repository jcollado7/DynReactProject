...
class LaunchAgent(Agent):
    class LABehav(OneShotBehaviour):
        async def run(self):
            global la_status_var, my_full_name, la_started_at, stop_time, my_dir, wait_msg_time, string_operations
            ...
            """Send new order to Browser"""
            if order_code != "No":
                print("Order to Browser running")
                launcher_to_browser_json = asf.order_to_browser(my_full_name, order_code, steel_grade, thickness, width_coils,
                                                    num_coils, list_coils, each_coil_price, string_operations)
                launcher_order_browser = asf.msg_to_browser(launcher_to_browser_json, my_dir)
                await self.send(launcher_order_browser)
