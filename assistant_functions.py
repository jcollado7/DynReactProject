...
def set_agent_parameters(agent_directory, agent_name, agent_full_name):
    ...
    if agent_name == 'va':
        agent_data = agent_data.reindex(columns=['id', 'agent_type', 'location_of_coils', 'purpose', 'request_type', 'time', 'activation_time', 'setup_speed', 'ancho', 'largo', 'espesor']) #Los valores ya existentes, se mantienen
        agent_data = va_parameters(agent_data, agents_df, agent_name)
    elif agent_name == "coil":
        agent_data = agent_data.reindex(
            columns=['id', 'agent_type', 'location', 'From', 'purpose', 'request_type', 'time', 'activation_time', 'to_do', 'number_auction', 'int_fab', 'bid', 'bid_status', 'ancho', 'largo', 'espesor', 'budget'])
...
def va_parameters(agent_data, agents_df, agent_name):
    ...
    agent_data.at[0, 'location_of_coils'] = agents_df.loc[0, 'location']
    return agent_data
...
def bids_mean(medias_list):
    ...
    print("LA MEDIA ES: ", medias_list)
    medias_list = stats.mean(medias_list)
    print("LA MEDIA ES: ", medias_list)
    return medias_list

def order_to_browser(agent_full_name, order_code, steel_grade, thickness, width_coils, num_coils, list_coils, each_coil_price,
               string_operations):
    va = string_operations.split(',')
    lc = list_coils.split(',')
    list = []
